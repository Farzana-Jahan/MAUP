...
moran.test(DATA_sf$sa3_count,nbp_w, zero.policy = TRUE)
# 
## Using MC simulation method to test for significance ##--------------------
MC<- moran.mc(DATA_sf$sa3_count,nbp_w, nsim=999, zero.policy = TRUE)
...
Sys.time() - start
save(model.BYM_noCov,file="BYM_SA3_noCov.RData")
...
DATA_sf$fitted_SIR_noCov<-result$fitted_values/DATA_sf$sa3_expect

# Plot observed versus fitted values
Fit_obs <- data.frame(observed=observed, fitted=model.ler$fitted.values)
