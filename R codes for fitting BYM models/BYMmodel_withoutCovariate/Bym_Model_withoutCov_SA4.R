...
##To add geometry, join SA3_raw_qld with SA3_level_count
DATA_SA4 <- SA4_level_count %>% 
  inner_join(.,SA4_raw_qld, by = c("SA4_CODE16"="SA4_CODE16"))

## Descriptive statistics ##
summary(DATA_SA4$sa4_count)
sd(DATA_SA4$sa4_count)


#Create simple feature object
DATA_sf <- st_sf(DATA_SA4)
...
DATA_sf$expected <- DATA_sf$sa4_expect
DATA_sf$expected[DATA_sf$expected==0] <- 0.001 

## Fit BYM model ##----------------------------------------------------------
start <- Sys.time()
formula <-observed ~ 1+ offset(log(expected))
model.BYM_noCov <- S.CARbym(formula=formula, data=DATA_sf, family="poisson", W=nbp_w,
                      burnin=500000, n.sample=1500000, thin=100)#10000 number of iterations

Sys.time() - start
save(model.BYM_noCov,file="BYM_SA4_noCov.RData")


## Summary of results (summary output of the fitted model) ##---------
print(model.BYM_noCov)
summary(model.BYM_noCov)

## Traceplots for parameters ##
plot(model.BYM_noCov$samples$tau2)
plot(model.BYM_noCov$samples$sigma2)
plot(model.BYM_noCov$samples$beta)
plot(model.BYM_noCov$residuals)
plot(model.BYM_noCov$fitted.values)

summary(model.BYM_noCov$samples)
summary(model.BYM_noCov$samples$psi)
summary(model.BYM_noCov$samples$sigma2)

# Combine residuals and fitted values into a data frame
result = data.frame(fitted_values = model.BYM_noCov$fitted.values, residuals = model.BYM_noCov$residuals)
DATA_sf$fitted_SIR_no_cov<-result$fitted_values/DATA_sf$sa4_expect

## Moran's I to the residuals (compute Moran's I statistic and test to assess its significance)##--------------------------------------------
res.model <- residuals(model.BYM_noCov)
nb <- poly2nb(DATA_sf, queen=FALSE)# nb lists all neighboring polygons
